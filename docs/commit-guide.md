...
### 12. Merge commits（合入 `main` / 其它分支）

Git 默认文案 `Merge branch 'main' into …` **不符合** 上文 schema（缺 emoji/type）。也不要自创不在 schema 内的 type（例如 `🔀 merge:`）。

合入上游时请写成 **`🧹 chore`**，并用 scope `merge` 标明：

```
🧹 chore(merge): integrate latest main into <topic>
```

- **描述**: 仅记录「把某分支合入当前分支」的 merge commit；不承载功能 diff 说明
- **语义化版本**: 无（merge 本身不映射版本）
- **示例**:
  ```
  🧹 chore(merge): integrate latest main into connection lifecycle
  🧹 chore(merge): integrate latest main into feat/redis-search
  ```

若使用 `git merge` 且不想编辑器弹窗，可：

```bash
git merge origin/main -m "🧹 chore(merge): integrate latest main into connection lifecycle"
```
...
ci            # CI 工作流
docs          # 文档
merge         # 合入 main/其它分支的 merge commit
```
...
10. **Merge commits**: 用 `🧹 chore(merge): …`，不要用默认 `Merge branch …` 或 schema 外的 `🔀 merge:`
...
# Merge 使用默认文案或未支持 type
git merge origin/main -m "Merge branch 'main' into feat/foo"
git merge origin/main -m "🔀 merge: integrate latest main"
```
