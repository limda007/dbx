...
pub async fn test_connection(
    ...
    let message =
        dbx_core::connection_lifecycle::test_connection(&state.app, body.config).await.map_err(AppError::from)?;
    Ok(Json(message))
}
...
pub async fn test_connection_with_info(
    ...
    let result = dbx_core::connection_lifecycle::test_connection_with_info(&state.app, body.config)
        .await
...
pub async fn connect_db(
    ...
    let connection_id = dbx_core::connection_lifecycle::connect(&state.app, config, body.client_attempt)
        .await
        .map_err(AppError::from)?;
...
mod tests {
    ...
    async fn save_connection_database_info_preserves_connected_pool() {
        ...
        state.app.insert_message_queue_pool_marker(&config.id).await;
    ...
    async fn save_connections_drops_cached_mq_adapter_for_updated_config() {
        ...
        state.app.configs.write().await.insert(initial.id.clone(), initial.clone());
        state.app.insert_message_queue_pool_marker(&initial.id).await;
        ...
        assert!(!state.app.has_pool(&initial.id).await);
    ...
    async fn save_connections_removes_deleted_connection_pools() {
        ...
        state.app.insert_message_queue_pool_marker(&removed.id).await;

        let result =
            save_connections(State(state.clone()), Json(SaveConnectionsRequest { configs: vec![kept.clone()] })).await;
        assert!(result.is_ok());

        assert!(!state.app.has_pool(&removed.id).await);
    ...
    async fn disconnect_db_keeps_connections_with_similar_prefixes() {
        ...
        state.app.insert_sqlite_pool("conn", conn_pool).await;
        state.app.insert_sqlite_pool("conn2", conn2_pool).await;
        ...
        assert!(!state.app.has_pool("conn").await);
        assert!(state.app.has_pool("conn2").await);
    ...
    async fn disconnect_db_drops_cached_mq_adapter() {
        ...
        assert!(!state.app.has_pool(&config.id).await);
