...
use crate::query::{await_stream_with_progress_timeout, StreamProgressClock};
...
pub async fn stream_select_query_with_cancel_logged(
    ...
    if result.as_ref().is_err_and(|error| error == &timeout_error || error == crate::query::QUERY_CANCELED) {
        cancel_postgres_query(pg_cancel_token, cancel_context.as_ref(), budget.cancel_timeout, log_context).await;
    }
