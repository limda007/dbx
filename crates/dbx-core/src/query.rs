...
pub(crate) fn schema_for_execution_context(db_type: Option<DatabaseType>, schema: Option<&str>) -> Option<&str> {
...
pub(crate) fn sql_for_execution_context(db_type: Option<DatabaseType>, sql: &str, schema: Option<&str>) -> String {
...
#[cfg(feature = "duckdb-bundled")]
pub(crate) async fn wait_for_duckdb_task_with_interrupt_outcome(
...
#[cfg(feature = "duckdb-bundled")]
pub(crate) async fn wait_for_duckdb_task_with_interrupt(
    cancel_token: Option<CancellationToken>,
    timeout_duration: Option<Duration>,
    interrupt_handle: std::sync::Arc<duckdb::InterruptHandle>,
...
pub(crate) async fn apply_oceanbase_mysql_session_timeout(
    state: &AppState,
    pool_key: &str,
...
pub async fn do_execute(
    ...
    let result = match crate::database_session::execute_sql(
        state,
        pool_key,
        mysql_dialect,
        database,
    ...
    .await
    {
        Err(err) if err.already_logged() => {
            return Err(err.into_message());
        }
        Err(err) if err.as_str() == "Connection not found" => {
...
pub(crate) async fn invoke_external_driver_query_page(
...
pub(crate) fn external_driver_fetch_query_page_params(
    config: &crate::models::connection::ConnectionConfig,
    session_id: &str,
    page_size: usize,
) -> serde_json::Value {
